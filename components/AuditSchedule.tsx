...
const AuditSchedule: FC<AuditScheduleProps> = ({ audits, onCreateAudit, onUpdateAudit, onDeleteAudit, onViewReport }) => {
  const { t, language } = useLanguage(); // Use language prop for date formatting
  ...
  const executeSaveSchedule = () => {
    ...
    // --- NOTIFICATION LOGIC ---
    const dateStr = startDate.toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US');
  ...
  const executeReschedule = () => {
    ...
    const dateStr = newDate.toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US');
  ...
  const monthName = calendarDate.toLocaleString(language === 'id' ? 'id-ID' : 'en-US', { month: 'long' });
        ...
        <div className="flex-none p-6 pb-4 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-50">
          <div>
            <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
              <CalendarDays className="text-blue-600" /> {t('sched.title')}
            </h2>
          ...
          <div className="flex items-center gap-2">
            <div className="bg-white border border-slate-300 rounded-lg p-1 flex items-center gap-1 shadow-sm mr-2">
               <button 
                  ...
                  title={t('sched.view.list')}
               >
                 <LayoutList size={18} />
                 <span className="text-xs hidden md:inline">{t('sched.view.list')}</span>
               </button>
               <button 
                  ...
                  title={t('sched.view.calendar')}
               >
                 <Calendar size={18} />
                 <span className="text-xs hidden md:inline">{t('sched.view.calendar')}</span>
               </button>
            </div>
                               ...
                               return (
                                ...
                                >
                                   ...
                                   <div className="opacity-80 text-[9px] truncate mt-0.5 flex items-center gap-1">
                                      ...
                                      {isOverdue ? t('sched.alert.overdue') : getStatusLabel(audit.status)}
                                   </div>
                                </div>
                             );
                            })}
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <h3 className="font-bold text-slate-800 flex items-center gap-2">
                        <Clock size={18} className="text-blue-500" /> {t('sched.upcoming')}
                    </h3>
                  ...
                  {canStartAudit && plannedAudits.some(a => a.status === AuditStatus.PLANNED) && (
                    ...
                    >
                       <Play size={14} fill="currentColor" /> {t('sched.btn.start_all')}
                    </button>
                  )}
                </div>
                {plannedAudits.length === 0 ? (
                  <div className="p-12 text-center flex flex-col items-center text-slate-400">
                    ...
                    <p>{t('sched.empty_upcoming')}</p>
                  </div>
                ) : (
                  <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 text-slate-500 font-medium">
                      <tr>
                        <th className="px-6 py-3">{t('dash.th.date')}</th>
                        <th className="px-6 py-3">{t('dash.th.dept')}</th>
                        <th className="px-6 py-3">{t('dash.th.status')}</th>
                        <th className="px-6 py-3">{t('dash.th.auditor')}</th>
                      ...
                      {plannedAudits.map(audit => {
                        ...
                        <tr key={audit.id} className={`hover:bg-slate-50 ${isOverdue ? 'bg-red-50/30' : ''}`}>
                          <td className="px-6 py-3 font-medium text-slate-700">
                            {new Date(audit.date).toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US')}
                            {isOverdue && (
                              <span className="ml-2 inline-flex items-center gap-1 bg-red-100 text-red-700 px-2 py-0.5 rounded-full text-[10px] font-bold border border-red-200">
                                <AlertCircle size={10} /> {t('sched.alert.overdue')}
                              </span>
                            )}
                          </td>
                          ...
                          <td className="px-6 py-3 text-right">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              {canStartAudit && audit.status === AuditStatus.PLANNED && (
                                ...
                                >
                                    <PlayCircle size={14} /> <span className="text-xs font-bold">{t('sched.action.start')}</span>
                                </button>
                              )}
                              ...
                              {canManageSchedule && (
                                <button 
                                    ...
                                    title={t('sched.action.reschedule')}
                                >
                                    <Calendar size={16} />
                                </button>
                              )}
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-6 py-4 border-b border-slate-100 bg-slate-50">
                  <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                     <LayoutList size={16} className="text-slate-400"/> {t('sched.history')}
                  </h3>
                </div>
                  ...
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {otherAudits.length === 0 ? (
                        <tr><td colSpan={4} className="p-6 text-center text-slate-400">{t('sched.empty_history')}</td></tr>
                      ) : (
                        otherAudits.map(audit => (
                          <tr key={audit.id} className="hover:bg-slate-50">
                            <td className="px-6 py-3 text-slate-500">{new Date(audit.date).toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US')}</td>
                            <td className="px-6 py-3 font-medium">{audit.department}</td>
                            <td className="px-6 py-3">
                            ...
                            <td className="px-6 py-3 text-right">
                              <div className="flex items-center justify-end gap-2">
                                 ...
                                 {canViewReports && (
                                    ...
                                    >
                                      <PieChart size={14} /> {t('repo.btn.view')}
                                    </button>
                                 )}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 className="font-bold text-slate-800">{t('sched.modal.new_title')}</h3>
              ...
              <form onSubmit={handleInitiateSchedule} className="p-6 space-y-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('sched.label.std')}</label>
                  <select 
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('sched.label.unit')}</label>
                  <select 
                    ...
                    className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                  >
                    <option value="">{t('new.placeholder.unit')}</option>
                    {units.map(u => (
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('sched.label.name')}</label>
                  <input 
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('sched.label.auditor')}</label>
                  <select 
                  ...
                  >
                    <option value="">
                      {!scheduleForm.department ? "-- Select Department First --" : t('new.placeholder.auditor')}
                    </option>
                    {users.filter(u => {
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1">{t('sched.label.date')}</label>
                  <input 
                    type="date"
                ...
                <div className="pt-2 flex gap-3">
                  <button type="button" onClick={() => setIsScheduleModalOpen(false)} className="flex-1 py-2 border rounded-lg text-slate-600 hover:bg-slate-50">{t('mgmt.btn.cancel')}</button>
                  <button type="submit" className="flex-1 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-bold shadow-md">{t('mgmt.btn.save')}</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {rescheduleState.isOpen && rescheduleState.audit && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-fade-in">
              <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                <h3 className="font-bold text-slate-800">{t('sched.modal.reschedule_title')}</h3>
              ...
              <form onSubmit={handleRescheduleSubmit} className="p-6 space-y-4">
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 text-xs text-blue-800 mb-2">
                   ...
                   <p>{new Date(rescheduleState.audit.date).toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US')}</p>
                </div>
                ...
                <div className="pt-2">
                    <button type="submit" className="w-full py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center justify-center gap-2 font-bold shadow-md">
                       <Save size={16} /> {t('mgmt.btn.update')}
                    </button>
                </div>
              </form>
            </div>
        ...
        {rescheduleConfirmOpen && (
           <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden p-6 text-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 mb-2">{t('confirm.general')}</h3>
                <p className="text-sm text-slate-500">
                  {language === 'id' ? 'Ubah jadwal untuk' : 'Reschedule audit for'} 
                  <span className="font-bold text-slate-700 block mt-1">{rescheduleState.audit?.department}</span>
                  {language === 'id' ? 'menjadi' : 'to'} <span className="font-bold text-blue-600">{new Date(rescheduleState.newDate).toLocaleDateString(language === 'id' ? 'id-ID' : 'en-US')}</span>?
                </p>
              </div>
        ...
        {createConfirmOpen && (
           <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-fade-in">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden p-6 text-center space-y-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-900 mb-2">{t('sched.confirm.title')}</h3>
                <p className="text-sm text-slate-500">
                  {t('sched.confirm.msg')}
                ...
                <div className="mt-2 bg-slate-50 p-2 rounded text-xs font-medium text-slate-600 border border-slate-100">
                   Status: <span className="text-slate-800 font-bold">{t('status.pending')}</span>
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                <button 
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm overflow-hidden p-6 text-center space-y-4">
               ...
               <div>
                  <h3 className="text-lg font-bold text-slate-900 mb-2">
                    {startConfirmModal.type === 'ALL' ? t('sched.btn.start_all') : t('sched.action.start') + '?'}
                  </h3>
                  <p className="text-sm text-slate-500">
                     Status audit akan berubah menjadi <strong>{t('status.progress')}</strong>.
                  </p>
               </div>
               <div className="flex gap-3 pt-2">
                  <button 
