  ...
  const openLink = (url: string, fileName?: string) => {
      ...
      if (url.startsWith('data:')) {
          try {
            ...
            // Check MIME Type for Open Behavior
            if (mime.includes('pdf') || mime.includes('image')) {
                link.target = '_blank';
            } else {
                // Force Download for office docs
                const dlName = fileName ? fileName.split('/').pop() : `file.${mime.split('/')[1] || 'bin'}`;
                link.download = dlName || 'download';
            }
                          ...
                          <div className="space-y-1">
                            {(q.evidence || q.evidenceFileName) && (
                               <div className="flex items-start gap-1">
                                  ...
                                  <button 
                                     onClick={() => openLink(q.evidence!, q.evidenceFileName)}
