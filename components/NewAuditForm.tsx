...
import { Sparkles, Loader2, ArrowRight, Plus, Trash2, Edit3, Save, ArrowLeft, GripVertical, AlertTriangle, Database, Bot, Filter, User, Lock, UserX, Send, CheckCircle2, FileText, CalendarClock, Type, FileType, Building2 } from 'lucide-react';
...
const NewAuditForm: FC<NewAuditFormProps> = ({ onAuditCreated, onCancel }) => {
  ...
  useEffect(() => {
    ...
    // Auto-fill name if empty or if it looks like a generated default name
    setAuditName(prev => {
       if (!prev || prev.includes('Audit Mutu')) {
           return `Audit Mutu ${department} - ${settings.auditPeriod}`;
       }
       return prev;
    });
  ...
  const executeSubmitAudit = async () => {
    ...
    const newAudit: AuditSession = {
      ...
      department,
      standard,
      status: AuditStatus.PENDING_SCHEDULING, // UPDATED: Changed from IN_PROGRESS
    ...
    // Notify Dept Head to Confirm
    const deptHeads = users.filter(u => u.department === department && u.role === UserRole.DEPT_HEAD);
    if (deptHeads.length > 0) {
       deptHeads.forEach(u => {
         addNotification(
           u.id,
           "Konfirmasi Jadwal Audit",
           `Audit baru untuk ${department} telah diajukan. Mohon konfirmasi jadwal agar audit dapat dimulai.`
         );
       });
    }
    ...
    if (assignedAuditorId) {
      addNotification(
        assignedAuditorId,
        "Penugasan Audit Tertunda",
        `Anda ditugaskan untuk audit ${department}. Menunggu konfirmasi jadwal dari Kepala Unit.`
      );
    }
    ...
    alert(`✅ Penugasan Berhasil Dibuat!\n\nStatus saat ini: PENDING SCHEDULE.\nMenunggu konfirmasi jadwal dari Kepala Unit (Dept.Head) ${department} agar audit dapat dimulai.`);
  };
